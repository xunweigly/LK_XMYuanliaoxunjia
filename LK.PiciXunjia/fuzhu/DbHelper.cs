        ...
        public static object ExecuteScalar(string sSql)
        {
            ...
            using (SqlConnection conna = new SqlConnection(conStr))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    DbHelper.WriteError(canshu.cMenuname2 + "-" + System.Reflection.MethodBase.GetCurrentMethod().Name, ex.Message, sSql);
                    //把异常抛到调用该方法的地方
                    //throw;
                }
                finally
        ...
        public static int ExecuteNonQuery(string SQLString, SqlParameter[] Paras)
        {
            using (SqlConnection connection = new SqlConnection(conStr))
            {
                using (SqlCommand cmd = new SqlCommand(SQLString, connection))
                {
                    ...
                    {
                        DbHelper.WriteError(canshu.cMenuname2 + "-" + System.Reflection.MethodBase.GetCurrentMethod().Name, e.Message, SQLString);
                        connection.Close();
                        throw e;
                    }
                }
        ...
        public static void WriteError(string cMenu, string cError,string cSql)
        {
           ...
           ,[cusername],sql) values(@cmenu,@cerror,getdate(),@ip,@cusername,@sql)";
            DbHelper.ExecuteNonQuery(sql, new SqlParameter[]{
                       ...
                       new SqlParameter("@ip",canshu.u8Login.cComputerName),
                       new SqlParameter("@cusername",canshu.userName),
